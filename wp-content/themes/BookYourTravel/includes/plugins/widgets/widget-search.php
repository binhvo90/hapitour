...
class bookyourtravel_search_widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$tour_tag_label_text = isset($instance['tour_type_label_text']) ? $instance['tour_type_label_text'] : esc_html__('Tour type', 'bookyourtravel');
				...
				<form class="widget-search" method="get" action="<?php echo esc_url($this->custom_search_results_page); ?>">
					...
					<div>
						...
						if ($this->enable_tours && $show_tour_type_filter)
							$this->render_tour_type_section($tour_tag_label_text);
	...
	function render_when_section($when_text, $accommodation_date_from_label_text, $accommodation_date_to_label_text, $car_rental_date_from_label_text, $car_rental_date_to_label_text, $tour_date_from_label_text, $cruise_date_from_label_text) {
		...
		$from = isset($_GET['from']) && !empty($_GET['from']) ? date('Y/m/d', strtotime(sanitize_text_field($_GET['from']))) : null;
		$to = isset($_GET['to']) && !empty($_GET['to']) ? date('Y/m/d', strtotime(sanitize_text_field($_GET['to']))) : null;
		?>
		<div class="column">
			<div class="when dt">
				<?php echo $when_text; ?>
	...
	function render_tour_type_section($tour_tag_label_text) {

		$request_type_ids = array();
		if (isset($_GET['tour_tags'])) {
			$request_type_ids = BookYourTravel_Theme_Utils::retrieve_array_of_values_from_query_string('tour_tags', true);
		}
		...
		$tour_tags = get_categories($args);

		if (count($tour_tags) > 0) {
			?>
			<div class="column">
				<div class="tour_type dt"><?php echo $tour_tag_label_text; ?></div>
				<div class="tour_type dd">
					<?php for ($i = 0; $i < count($tour_tags); $i++) {
						if (isset($tour_tags[$i])) {
							$tour_tag = $tour_tags[$i];

							$checked = '';
							if (in_array($tour_tag->term_id, $request_type_ids)) {
								$checked = " checked='checked' ";
							}
							?>
							<div class="checkbox">
								<input <?php echo $checked; ?> value="<?php echo esc_attr( $tour_tag->term_id ); ?>" type="checkbox" id="ct<?php echo $i + 1; ?>" name="tour_tags[]" />
								<label for="ct<?php echo $i + 1; ?>"><?php echo esc_attr( $tour_tag->name ); ?></label>
							</div>
						<?php }
					}?>
				</div>
