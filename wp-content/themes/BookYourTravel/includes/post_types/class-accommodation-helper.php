...
//						}
	...
	function get_accommodation_price($accommodation_id = 0, $room_type_id = 0, $start_date = null, $end_date = null, $location_id = 0) {
		...
		if ($price == -1) {
			...
			$sql = "
				SELECT MIN(price_per_day) price_per_day
				FROM
				(
					SELECT a.vacancy_id, a.price_per_day, a.price_per_day_child, a.weekend_price_per_day, a.weekend_price_per_day_child, a.room_count, a.booked_rooms, 
					(@runtot := @runtot + a.room_count) AS running_available_total
					FROM
					(
						SELECT  availables.vacancy_id, availables.price_per_day, availables.price_per_day_child, availables.weekend_price_per_day, availables.weekend_price_per_day_child, IFNULL(SUM(bookings.room_count), 0) booked_rooms, IFNULL(SUM(availables.room_count), 0) room_count 
						FROM 
						(
						SELECT availables_inner.*, date_format(DATE(availables_inner.single_date), '%Y-%m-%d 12:00:01') as bookable_single_date ";
			...
			$sql .= $wpdb->prepare("FROM
							(
								SELECT vacancies.Id vacancy_id, dates.day single_date, vacancies.price_per_day, vacancies.price_per_day_child, IFNULL(vacancies.weekend_price_per_day, 0) weekend_price_per_day, IFNULL(vacancies.weekend_price_per_day_child, 0) weekend_price_per_day_child, vacancies.room_count
								FROM " . BOOKYOURTRAVEL_ACCOMMODATION_VACANCIES_TABLE . " vacancies
								INNER JOIN " . BOOKYOURTRAVEL_ACCOMMODATION_VACANCIES_DAYS_TABLE . " dates ON dates.vacancy_id = vacancies.Id
								WHERE %s <= dates.day AND %s > dates.day ", $start_date, $end_date );
			...
			$sql .= " 
							GROUP BY vacancy_id, single_date
						) availables_inner
					) availables
					LEFT JOIN " . BOOKYOURTRAVEL_ACCOMMODATION_BOOKINGS_TABLE . " bookings ON availables.bookable_single_date BETWEEN bookings.date_from AND bookings.date_to ";
