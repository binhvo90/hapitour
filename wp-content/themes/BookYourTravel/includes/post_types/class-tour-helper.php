...
class BookYourTravel_Tour_Helper extends BookYourTravel_BaseSingleton {
		...
		if ( isset($wp_query->query_vars['post_type']) && $wp_query->query_vars['post_type'] == 'tour' ) {
			...
			if ($search_only_available || isset($wp_query->query_vars['byt_date_from']) || isset($wp_query->query_vars['byt_date_from'])) {
				...
				if (isset($date_from)) {
					...
					$fields .= "	INNER JOIN " . BOOKYOURTRAVEL_TOUR_SCHEDULE_DAYS_TABLE . " days ON days.schedule_id=schedule.id AND (days.day < schedule.end_date OR schedule.end_date IS NULL OR schedule.end_date = '0000-00-00 00:00:00')";
					...
					if ($date_from != null) {
						$fields .= $wpdb->prepare( " AND ( ( DATE(%s) <= DATE(start_date) AND (end_date IS NULL OR end_date = '0000-00-00 00:00:00') ) OR ( %s < end_date	) )	", $date_from, $date_from);
					}
	...
	function delete_tour_booking($booking_id) {
		...
		$schedule = $this->get_tour_schedule($booking->tour_schedule_id);
		$this->clear_price_meta_cache($schedule->tour_id);
	...
	function get_tour_available_schedule_id($tour_id, $date) {
		...
		$sql = "SELECT MIN(id) schedule_id
				...
				WHERE tour_id=%d AND schedule.max_people > (
					SELECT COUNT(*) ct 
					FROM $table_name_bookings bookings 
					INNER JOIN $table_name_schedule schedule2 ON bookings.tour_schedule_id = schedule2.Id
					WHERE schedule2.tour_id = %d AND bookings.tour_schedule_id = schedule.Id AND bookings.tour_date = %s ";
		...
		if ($tour_obj->get_type_is_repeated() == 0) {
			...
			$sql .= " AND %s >= schedule.start_date AND (%s < schedule.end_date OR schedule.end_date IS NULL OR schedule.end_date = '0000-00-00 00:00:00') ";
		}

		$sql = $wpdb->prepare($sql, $tour_id, $tour_id, $date, $date, $date);
	...
	function get_tour_min_price($tour_id, $date = null) {
		...
		if ($min_price == -1) {
			...
			if (isset($date)) {
				...
				if ($tour_type_is_repeated > 0) {
					$sql .= ' AND DATE(schedule_days.day) < schedule.end_date ';
				}
			}
