...
class BookYourTravel_Tour_Bookings_Admin extends BookYourTravel_BaseSingleton {
	...
	function bookings_admin_page() {
		...
		$hook = add_submenu_page('edit.php?post_type=tour', esc_html__('Tour Bookings', 'bookyourtravel'), esc_html__('Bookings', 'bookyourtravel'), 'edit_posts', basename(__FILE__), array( $this, 'bookings_admin_display') );
	...
	function bookings_admin_styles() {
		...
		echo '.wp-list-table .column-TourName { width: 20%; }';
		echo '.wp-list-table .column-TourDate { width: 12%; }';
	...
	function bookings_add_screen_options() {
		...
		global $wp_tour_bookings_table;
		$option = 'per_page';
		$args = array('label' => esc_html__('Bookings', 'bookyourtravel'),'default' => 50,'option' => 'bookings_per_page');
		add_screen_option( $option, $args );
		$wp_tour_bookings_table = new Tour_Bookings_Admin_List_Table();
	}

	function bookings_admin_display() {
	
		echo '</pre><div class="wrap">';
		echo '<h2>' . esc_html__('Tour bookings', 'bookyourtravel') . '</h2>';
		
		global $wp_tour_bookings_table;

		$wp_tour_bookings_table->handle_form_submit();
		...
		global $bookyourtravel_tour_helper, $booking_insert_success;
		...
		if (!empty($booking_id)) {
			...
			if (isset($_POST['tour_id']) && !$booking_insert_success) {
				$tour_id = (int)$_POST['tour_id'];
			}
			if (isset($_POST['tour_schedule_id']) && !$booking_insert_success) {
				$tour_schedule_id = (int)$_POST['tour_schedule_id'];
			}
		}
		...
		if ($tour_id > 0) {
			...
			echo 'window.tourId = ' . $tour_id . ';';
			echo 'window.tourScheduleId = ' . $tour_schedule_id . ';';
			echo 'window.isPricePerGroup = ' . json_encode($is_price_per_group) . ';';
		}

		echo '</script>';	
		
		if (isset($_GET['view'])) {
			$wp_tour_bookings_table->render_view_form(); 
		} else if (isset($_GET['sub']) && $_GET['sub'] == 'manage') {
			$wp_tour_bookings_table->render_entry_form($booking_id); 
		} else {	
			$wp_tour_bookings_table->prepare_items(); 
			...
			<div class="alignright actions ">
				<form method="get" action="<?php echo esc_url($form_uri); ?>">
					...
					<?php
					$wp_tour_bookings_table->search_box( 'search', 'search_id' );
					?>
				</form>
			</div>
			<?php	
			$wp_tour_bookings_table->display(); 
		} 
	}
}

global $tour_bookings_admin;
$tour_bookings_admin = BookYourTravel_Tour_Bookings_Admin::get_instance();
 ...
 */
class Tour_Bookings_Admin_List_Table extends WP_List_Table {
	...
	*/
	...
	function extra_tablenav( $which ) {
		...
		<div class="alignleft actions bookyourtravel-admin-top">
			<a href="edit.php?post_type=tour&page=theme_tour_schedule_booking_admin.php&sub=manage" class="button-secondary action" ><?php esc_html_e('Add Booking', 'bookyourtravel') ?></a>
		</div>
		<?php
		}
		if ( $which == "bottom" ) {
	...
	function column_Tour($item) {
		return $item->tour_name . (isset($item->room_type) ? '<br />' . $item->room_type : '');	
	}
	...
	function column_Action($item) {
		return  "<a href='edit.php?post_type=tour&page=theme_tour_schedule_booking_admin.php&sub=manage&booking_id=" . $item->Id . "'>" . esc_html__('Edit', 'bookyourtravel') . "</a> | 		
	...
	function get_columns() {
		return $columns= array(
			...
			'TourDate'=>esc_html__('Tour date', 'bookyourtravel'),
			'Tour'=>esc_html__('Tour', 'bookyourtravel'),
	...
	public function get_sortable_columns() {
		$sortable_columns = array(
			'Id'=> array( 'Id', true ),
			'Tour'=> array( 'tours.post_title', true ),
			'TotalPrice'=> array( 'total_price', true ),
			'TourDate'=> array( 'date_from', true ),
	...
	function prepare_items() {
		...
		global $bookyourtravel_tour_helper, $bookyourtravel_room_type_helper;
		...
		$tour_booking_results = $bookyourtravel_tour_helper->list_tour_bookings($paged, $per_page, $orderby, $order, $search_term, 0, $author_id);		
		
		//Number of elements in your table?
		$totalitems = $tour_booking_results['total']; //return the total number of affected rows
		...
		/* -- Fetch the items -- */
		$this->items = $tour_booking_results['results'];
	}
	
	function handle_form_submit() {
		
		global $bookyourtravel_tour_helper, $bookyourtravel_theme_globals, $bookyourtravel_theme_woocommerce, $booking_insert_success;
		...
		if (isset($_POST['delete_booking']) && check_admin_referer('bookyourtravel_nonce')) {
			...
			if (!empty($error)) {
				...
				if(empty($_POST['tour_date'])) {
					$error = esc_html__('You must select a tour date', 'bookyourtravel');
				...
				if (!empty($error)) {
					...
					$booking_object = $bookyourtravel_tour_helper->retrieve_booking_values_from_request();
				
					if (isset($_POST['insert'])) {
						
						$booking_object->Id = $bookyourtravel_tour_helper->create_tour_booking($current_user->ID, $booking_object);
						...
						if (BookYourTravel_Theme_Utils::is_woocommerce_active() && $use_woocommerce_for_checkout && !$tour_is_reservation_only) {
							$bookyourtravel_theme_woocommerce->dynamically_create_tour_woo_order($booking_object->Id, $booking_object->total_price, $address_array, $tour_id);
						}
						
						if ($booking_object->Id > 0) {
							echo '<div class="updated" id="message" onclick="this.parentNode.removeChild(this)">';
							echo '<p>' . esc_html__('Successfully inserted new tour booking entry!', 'bookyourtravel') . '</p>';
							echo '</div>';
							$booking_insert_success = true;
						} else {
							echo '<div class="error" id="message" onclick="this.parentNode.removeChild(this)">';
							echo '<p>' . esc_html__('Oops. Something happened! Failed to insert new tour booking entry!', 'bookyourtravel') . '</p>';
							echo '</div>';						
						}
						...
						$result = $bookyourtravel_tour_helper->update_tour_booking ($booking_id, $booking_object);

						if ($result == 1) {
							echo '<div class="updated" id="message" onclick="this.parentNode.removeChild(this)">';
							echo '<p>' . esc_html__('Successfully updated tour booking entry!', 'bookyourtravel') . '</p>';
							echo '</div>';
						} else {
							echo '<div class="error" id="message" onclick="this.parentNode.removeChild(this)">';
							echo '<p>' . esc_html__('Oops. Something happened! Failed to update tour booking entry!', 'bookyourtravel') . '</p>';
							echo '</div>';	
						}
	...
	function render_entry_form($booking_id) {
		...
		global $booking_insert_success, $bookyourtravel_tour_helper, $enable_extra_items, $bookyourtravel_theme_globals;
		...
		} else if (isset($_POST['tour_id']) && !$booking_insert_success ) {
			$tour_id = intval(sanitize_text_field($_POST['tour_id']));
		...
		} else if (isset($_POST['tour_schedule_id']) && !$booking_insert_success) {
			$tour_schedule_id = intval(sanitize_text_field($_POST['tour_schedule_id']));
		} else if ($booking_object != null) {
			$tour_schedule_id = $booking_object->tour_schedule_id;
		}
		...
		$tour_date = null;
		$tour_date_formatted = '';
		if (isset($_POST['tour_date']) && !$booking_insert_success)
			$tour_date = sanitize_text_field($_POST['tour_date']);
		else if ($booking_object != null) {
			$tour_date = $booking_object->tour_date;
		}
		if (isset($tour_date))
			$tour_date_formatted = date( $this->date_format, strtotime( $tour_date ) );
		...
		$total_tour_price = 0;
		if (isset($_POST['total_tour_price']) && !$booking_insert_success)
			$total_tour_price = floatval(sanitize_text_field($_POST['total_tour_price']));
		else if ($booking_object != null) {
			$total_tour_price = $booking_object->total_tour_price;
		}
				
		if ($booking_object)
			echo '<h3>' . esc_html__('Update Tour Booking', 'bookyourtravel') . '</h3>';
		else
			echo '<h3>' . esc_html__('Add Tour Booking', 'bookyourtravel') . '</h3>';
			
		if ($booking_id > 0 && BookYourTravel_Theme_Utils::is_woocommerce_active() && $tour_id > 0) {
			echo esc_html__('Please note: you are unable to edit certain fields of existing bookings because you are using WooCommerce for payment processing. In order to keep data in sync between the book your travel bookings table and WooCommerce orders, and to prevent data corruption or loss, fields like tour, room type, dates to and from, numbers of rooms, adults and children as well as the total price of a booking cannot be edited.', 'bookyourtravel');
		}		

		echo '<form id="tour_booking_form" method="post" action="' . esc_url($_SERVER['REQUEST_URI']) . '" style="clear: both;">';
		...
		if ($booking_id > 0 && BookYourTravel_Theme_Utils::is_woocommerce_active() && $tour_id > 0) {
			// we are editing a booking and woocommerce is active, so don't allow change of tour because data will be corrupt then (data sync between woocommerce order and byt booking).
			
			$tours_control .= "<input type='hidden' value='" . $tour_id . "' id='tour_id' name='tour_id' />";
			$tours_control .= $tour_obj->get_title();
			
		} else {
		
			$tour_results = $bookyourtravel_tour_helper->list_tours(0, -1, 'title', 'ASC', 0, array(), array(), array(), false, $author_id);

			$tours_control = '<select id="tour_id" name="tour_id" class="booking_tours_select">';
			$tours_control .= '<option value="">' . esc_html__('Select tour', 'bookyourtravel') . '</option>';
			if ( count($tour_results) > 0 && $tour_results['total'] > 0 ) {
				foreach ($tour_results['results'] as $tour_result) {
					$tours_control .= '<option value="' . $tour_result->ID . '" ' . ($tour_result->ID == $tour_id ? 'selected' : '') . '>' . $tour_result->post_title . '</option>';
				}
			}
			$tours_control .= '</select>';
		}
		
		echo '<tr>';
		echo '	<th scope="row" valign="top">' . esc_html__('Tour', 'bookyourtravel') . '</th>';
		echo '	<td>' . $tours_control . '</td>';
		echo '</tr>';
		
		echo "<input type='hidden' value='" . $tour_schedule_id . "' id='tour_schedule_id' name='tour_schedule_id' />";					
		
		echo '<tr style="display:none;" class="tour_selected step_1">';
		echo '	<th scope="row" valign="top">' . esc_html__('Tour date', 'bookyourtravel') . '</th>';
		echo '	<td>';
		
		if ($booking_id > 0) {
			echo '<span class="tour_date_span">' . $tour_date_formatted . '</span>';
		} else {		
			echo '		<script>';
			echo '			window.datepickerTourDateValue = "' . (isset($tour_date_formatted) ? $tour_date_formatted : null) . '";';
			echo '  	</script>';				
			echo '  	<input class="datepicker" type="text" name="booking_datepicker_tour_date" id="booking_datepicker_tour_date" />';
		} 
		echo '		<input type="hidden" name="tour_date" id="tour_date" value="' . $tour_date . '" />';
		echo '	</td>';	
		echo '</tr>';
		...
		foreach ($booking_form_fields as $booking_field) {
			...
			if (!$field_hidden) {			
				...
				echo '<tr style="display:none;" class="tour_selected step_2">';
		...
		if ($enable_extra_items) {
			...
			if (isset($booking_object->extra_items)) {
				...
				if ($extra_items_array && count($extra_items_array) > 0) {
					...
					foreach ($extra_items_array as $extra_item_id => $quantity) {
						...
						$tour_extra_item_obj = new BookYourTravel_Extra_Item($extra_item_id);
						...
						echo esc_html($tour_extra_item_obj->get_title());
						echo "</td>";
						echo "<td>";
						echo $quantity;
						echo "</td>";
			...
			echo '	<td><input type="hidden" name="total_tour_price" id="total_tour_price" value="' . $total_tour_price . '" /><span class="total_tour_price">' . $this->format_price($total_tour_price) . '</span></td>';		
			echo '</tr>';
		...
		echo '</table>';
		echo '<p>';
		echo '<a href="edit.php?post_type=tour&page=theme_tour_schedule_booking_admin.php" class="button-secondary">' . esc_html__('Cancel', 'bookyourtravel') . '</a>&nbsp;';
		if ($booking_object) {
			echo '<span style="display:none;" class="tour_selected step_2">';
