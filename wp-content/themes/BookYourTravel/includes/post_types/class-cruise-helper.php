		...
		if ( isset($wp_query->query_vars['post_type']) && $wp_query->query_vars['post_type'] == 'cruise' ) {
			...
			if ($search_only_available || isset($wp_query->query_vars['byt_date_from']) || isset($wp_query->query_vars['byt_date_from'])) {
				...
				if (isset($date_from)) {
					...
					$fields .= "	INNER JOIN " . BOOKYOURTRAVEL_CRUISE_SCHEDULE_DAYS_TABLE . " days ON days.schedule_id=schedule.id AND (days.day < schedule.end_date OR schedule.end_date IS NULL OR schedule.end_date = '0000-00-00 00:00:00')";
